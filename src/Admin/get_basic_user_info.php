<?php
	...
	if(isset($isAuthenticated['error'])){
		...
		$query = 'Select plan, created, expires, accountState from plans INNER JOIN users ON plans.userId=users.UserId where user=?';
		try{
			...
			$stmt->execute([$_GET['userListId']]);
				...
				if($stmt->rowCount() == 0){
					$query = 'Select accountState from users where user=?';
					$stmt = $conn->prepare($query);
					$stmt->execute([$_GET['userListId']]);
					if( $stmt->rowCount() == 1){
						$row = $stmt->fetch();
						$response = [
							...
							'info' => 'User still doesn\'t have a plan (in life)'
						];
